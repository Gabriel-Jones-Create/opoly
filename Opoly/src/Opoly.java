...
public class Opoly {
	...
	/**
	 * Changes the position of the character based on the remainder of the new position divided by the board size
	 * @param spinResult takes a number that is added to the position variable.
	 */
	private void move(int spinResult) {
		// if the new position of the character exceeds the size of the board, then it would pass the beginning meaning that 100 would be added to the balance 
	 ...
	 * @return returns <code>true</code> if reward exceeds or is equal to 1000. Otherwise returns <code>false</code>
	 */
	private boolean isGameOver() {
		// if the reward is greater than 1000 the game would be over, otherwise the game would continue
		if(reward >= 1000) {
			return true;
		}
		else {
	...
	private void drawBoard() {
		...
		for(int i = 0; i < boardSize; i++) {
			// if the position is equal to the iteration that its on, then it prints an "o", otherwise it prints a "*"
			if(position == i) {
				System.out.print("o");
			}
			else {
